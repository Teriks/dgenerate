...
import dgenerate.pipelinewrapper.uris.scascadedecoderuri as _scascadedecoderuri
from dgenerate.pipelinewrapper.uris.exceptions import InvalidSCascadeDecoderUriError


class TestSCascadeDecoderUri(unittest.TestCase):

    def test_basic_parsing(self):
        # Test basic model path parsing
        uri = "path/to/model"
        result = _scascadedecoderuri.SCascadeDecoderUri.parse(uri)
    ...
    def test_dtype_validation(self):
        ...
        # Invalid dtype
        with self.assertRaises(InvalidSCascadeDecoderUriError) as context:
            _scascadedecoderuri.SCascadeDecoderUri.parse("model;dtype=invalid_dtype")
