...
.. _vermeer_canny_edged.png_1: https://raw.githubusercontent.com/Teriks/dgenerate/v4.4.2/examples/media/vermeer_canny_edged.png
...
.. |Documentation| image:: https://readthedocs.org/projects/dgenerate/badge/?version=v4.4.2
   :target: http://dgenerate.readthedocs.io/en/v4.4.2/
...
please visit `readthedocs <http://dgenerate.readthedocs.io/en/v4.4.2/>`_.

----

* `Help Output`_
* `Diffusion Feature Table <https://github.com/Teriks/dgenerate/blob/v4.4.2/FEATURE_TABLE.rst>`_
...
.. code-block:: text
    ...
    options:
      ...
      -mc EXPR [EXPR ...], --cache-memory-constraints EXPR [EXPR ...]
                            ...
                            See: [https://dgenerate.readthedocs.io/en/v4.4.2/dgenerate_submodules.html#dgenerate
      ...
      -pmc EXPR [EXPR ...], --pipeline-cache-memory-constraints EXPR [EXPR ...]
                            ...
                            te.readthedocs.io/en/v4.4.2/dgenerate_submodules.html#dgenerate.pipelinewrapper.PIPE
      ...
      -umc EXPR [EXPR ...], --unet-cache-memory-constraints EXPR [EXPR ...]
                            ...
                            n/v4.4.2/dgenerate_submodules.html#dgenerate.pipelinewrapper.UNET_CACHE_MEMORY_CONST
                            RAINTS]
                            -------
      -vmc EXPR [EXPR ...], --vae-cache-memory-constraints EXPR [EXPR ...]
                            ...
                            /v4.4.2/dgenerate_submodules.html#dgenerate.pipelinewrapper.VAE_CACHE_MEMORY_CONSTRA
                            INTS]
                            -----
      -cmc EXPR [EXPR ...], --control-net-cache-memory-constraints EXPR [EXPR ...]
                            ...
                            generate.readthedocs.io/en/v4.4.2/dgenerate_submodules.html#dgenerate.pipelinewrappe
      ...
      -tmc EXPR [EXPR ...], --text-encoder-cache-memory-constraints EXPR [EXPR ...]
                            ...
                            [https://dgenerate.readthedocs.io/en/v4.4.2/dgenerate_submodules.html#dgenerate.pipe
      ...
      -amc EXPR [EXPR ...], --adapter-cache-memory-constraints EXPR [EXPR ...]
                            ...
                            ps://dgenerate.readthedocs.io/en/v4.4.2/dgenerate_submodules.html#dgenerate.pipeline
      ...
      -ipmc EXPR [EXPR ...], --image-processor-memory-constraints EXPR [EXPR ...]
                            ...
                            Syntax See: [https://dgenerate.readthedocs.io/en/v4.4.2/dgenerate_submodules.html#dg
...
.. code-block:: bash
    ...
    pipx install dgenerate[ncnn]==4.4.2 ^
    --pip-args "--extra-index-url https://download.pytorch.org/whl/cu124/"

    # You can install without pipx into your own environment like so

    pip install dgenerate==4.4.2 --extra-index-url https://download.pytorch.org/whl/cu124/

    # Or with NCNN

    pip install dgenerate[ncnn]==4.4.2 --extra-index-url https://download.pytorch.org/whl/cu124/
...
.. code-block:: bash
    ...
    pip3 install dgenerate==4.4.2 --extra-index-url https://download.pytorch.org/whl/cu124/

    # Or with NCNN

    pip3 install dgenerate[ncnn]==4.4.2 --extra-index-url https://download.pytorch.org/whl/cu124/
...
You must also use ``--force-reinstall`` with a link to the wheel for a development version of ``bitsandbytes`` that supports ROCm.

This wheel can be located on the multi-backend preview `release page <https://github.com/bitsandbytes-foundation/bitsandbytes/releases/tag/continuous-release_multi-backend-refactor>`_,
...
.. code-block:: bash
    ...
    # not explicitly specify a dgenerate version, i.e. you can only
    # grab the latest version of dgenerate from PyPi with these commands

    # if you need a specific version of dgenerate you will need to install
    # into your own environment instead of using pipx, and then install
    # the multi-backend preview of bitsandbytes over the top of your
    # dgenerate install with a second invocation of pip
...
.. code-block:: bash
    ...
    !source /content/venv/bin/activate; pip install dgenerate==4.4.2 --extra-index-url https://download.pytorch.org/whl/cu121
 ...
 * `my-image-seed.png <https://raw.githubusercontent.com/Teriks/dgenerate/v4.4.2/examples/media/dog-on-bench.png>`_
 * `my-mask-image.png <https://raw.githubusercontent.com/Teriks/dgenerate/v4.4.2/examples/media/dog-on-bench-mask.png>`_
...
This Stable Cascade functionality is demonstrated in the example config: `examples/stablecascade/img2img/multiple-inputs-config.dgen <https://github.com/Teriks/dgenerate/blob/v4.4.2/examples/stablecascade/img2img/multiple-inputs-config.dgen>`_
...
This image of a `horse <https://raw.githubusercontent.com/Teriks/dgenerate/v4.4.2/examples/media/horse2.jpeg>`_
...
The image used in the example below is this `low resolution cat <https://raw.githubusercontent.com/Teriks/dgenerate/v4.4.2/examples/media/low_res_cat.png>`_
...
in the `writing_plugins/image_processor <https://github.com/Teriks/dgenerate/tree/v4.4.2/examples/writing_plugins/image_processor>`_
folder of the examples folder.

The source code for the built in `canny <https://github.com/Teriks/dgenerate/blob/v4.4.2/dgenerate/imageprocessors/canny.py>`_ processor,
the `openpose <https://github.com/Teriks/dgenerate/blob/v4.4.2/dgenerate/imageprocessors/openpose.py>`_ processor, and the simple
`pillow image operations <https://github.com/Teriks/dgenerate/blob/v4.4.2/dgenerate/imageprocessors/imageops.py>`_ processors can also
...
An example for writing config directives can be found in the `writing_plugins/config_directive <https://github.com/Teriks/dgenerate/tree/v4.4.2/examples/writing_plugins/config_directive>`_  example folder.

Config template functions can also be implemented by plugins, see: `writing_plugins/template_function <https://github.com/Teriks/dgenerate/tree/v4.4.2/examples/writing_plugins/template_function>`_

Currently the only internal directive that is implemented as a plugin is the ``\image_process`` directive, who's source file 
`can be located here <https://github.com/Teriks/dgenerate/blob/v4.4.2/dgenerate/batchprocess/image_process_directive.py>`_.
...
`is implemented here <https://github.com/Teriks/dgenerate/blob/v4.4.2/dgenerate/image_process>`_.
...
Reference for writing sub-commands can be found in the `image-process <https://github.com/Teriks/dgenerate/blob/v4.4.2/dgenerate/subcommands/image_process.py>`_
sub-command implementation, and a plugin skeleton file for sub-commands can be found in the 
`writing_plugins/sub_command <https://github.com/Teriks/dgenerate/tree/v4.4.2/examples/writing_plugins/sub_command>`_ example folder.
...
Reference for writing prompt weighters can be found in the `CompelPromptWeighter <https://github.com/Teriks/dgenerate/blob/v4.4.2/dgenerate/promptweighters/compelpromptweighter.py>`_
and `SdEmbedPromptWeighter <https://github.com/Teriks/dgenerate/blob/v4.4.2/dgenerate/promptweighters/sdembedpromptweighter.py>`_ internal prompt weighter implementations.
 
A plugin skeleton file for prompt weighters can be found in the 
`writing_plugins/prompt_weighter <https://github.com/Teriks/dgenerate/tree/v4.4.2/examples/writing_plugins/prompt_weighter>`_
example folder.
